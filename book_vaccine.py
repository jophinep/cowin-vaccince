...
def book_now():
    ...
    for i in data["centers"]:
        if i["fee_type"] == "Free" and i["block_name"] == "Thiruvananthapuram":
            for j in i["sessions"]:
                if j["vaccine"] == "COVISHIELD" and (j["min_age_limit"] == 18 or j["allow_all_age"] is True) and j["available_capacity_dose2"] != 0:
                    print(i["name"])
                    print(i["address"])
                    print(j["slots"][0])
                    print(j["available_capacity_dose2"])
...
# keep_alive = input("Should I logout? ")
